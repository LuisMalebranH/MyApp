...
export class LoginPage {
  ...
  password: string = '';
  @ViewChild(IonInput, { read: ElementRef }) inputField: ElementRef<HTMLIonInputElement>;
 ...
 /* login:any={
  ...
  constructor(public router:Router, public toastController:ToastController, private authService: AuthService,private animationCtrl: AnimationController) { }
  ...
  async login() {
    ...
    ngAfterViewInit() {
      this.animation = this.animationCtrl
        .create()
        .addElement(this.inputField.nativeElement)
        .duration(1500)
        .iterations(Infinity)
        .direction('alternate')
        .fromTo('background', 'blue', 'var(--background)'); // Cambia el color de fondo
    }
    play() {
      this.animation.play();
    }
